...
spell_check_tokenizer_2 = AutoTokenizer.from_pretrained("ai-forever/sage-fredt5-distilled-95m")
spell_check_model_2 =  AutoModelForSeq2SeqLM.from_pretrained("ai-forever/sage-fredt5-distilled-95m")

spell_check_model.to("cuda")
spell_check_model_2.to("cuda")
...
for i in 'abejr':
    for j in range(5):
        ...
        inputs = spell_check_tokenizer_2(transcription, max_length=None, padding="longest", truncation=False, return_tensors="pt")
        outputs = spell_check_model_2.generate(**inputs.to(spell_check_model_2.device), max_length = inputs["input_ids"].size(1) * 1.5)
        correct_text = preprocess_text(spell_check_tokenizer_2.batch_decode(outputs, skip_special_tokens=True)[0])

        inputs = spell_check_tokenizer(correct_text, return_tensors="pt").to("cuda")
