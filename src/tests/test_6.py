...
def super_preprocess_text(text):
...
for i in 'abejr':
    for j in range(5):
        raw_standard = standards[j]
        standard =super_preprocess_text(standards[j])
        inputs = spell_check_tokenizer(standard, max_length=None, padding="longest", truncation=False, return_tensors="pt")
        outputs = spell_check_model.generate(**inputs.to(spell_check_model.device), max_length = inputs["input_ids"].size(1) * 1.5)
        fredt5_text = preprocess_text(spell_check_tokenizer.batch_decode(outputs, skip_special_tokens=True)[0])
        results: List[List[str]] = punctuation_model.infer(
            texts=list([standard]), apply_sbd=False
        )
        roberta_text = results[0]
        roberta_cer = fastwer.score_sent(roberta_text, raw_standard,  char_level=True) * 0.01
        fredt5_cer = fastwer.score_sent(fredt5_text, raw_standard, char_level=True) * 0.01
        total_roberta_cer += roberta_cer
        total_fredt5_cer += fredt5_cer
        report = f"Запись: {i}{j+1}.opus\nRoberta CER: {roberta_cer}\nSage-fredt5-distilled-95m CER: {fredt5_cer}\n-------------------------------\n"
        result += report

print(result)
print(f"Средний Roberta CER:{total_roberta_cer/25}\nСредний Sage-m2m100-1.2B CER: {total_fredt5_cer/25}")
